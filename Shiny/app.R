...
#use_python("/usr/local/bin/python2.7", required = TRUE)
library(shiny)
library(Seurat)
library(ggplot2)
library(dplyr)
...
# Sidebar panel for inputs ----
  ...
  # Input: Select a file ----
    fileInput("seuratFile", "Upload Data File",
              multiple = FALSE,
              accept = c(".rds")),
                ...
                type = "tabs",
                  tabPanel("Getting started",
                        value = "panel1",
                         ...
                         p("1. Upload the data.rds file and image.rds files. It can take a couple of minutes for the data to upload"),
                         p("2. Select the type of plot you want to look at. There are 4 plots available showing a) the
      number of genes detected per spot, b) the number of reads detected per spot, c) clustering results,
      d) the gene expression of a selected gene."),
                         p("3. For the cluster plot you must also select the cluster resolution you wish to plot
      (generally a lower resolution equates to fewer clusters."),
                         p("4. For the gene plot you must select a gene from the drop downlist. There is a bit of a delay whilst the gene list is loading.
      You can jump to the gene in list by typing the first few letters of the gene of interest."), 
...
server <- function(input, output, session) {
  ...
  output$summary <- renderPrint({
    "1. Upload the data.rds file and image.rds files. It can take a couple of minutes for the data to upload"
  })
  
  
  ### Seurat object
  seuratObj <- reactive({
    req(input$seuratFile)
    readRDS(input$seuratFile$datapath)
  })
  ...
  clusterList <- reactive({
    req(seuratObj())  
    colnames(seuratObj()@meta.data)[grep("res", colnames(seuratObj()@meta.data))]
    })
